...
- If the file opens with a hero image, HTML banner, or short intro block, place the selector right
  after that opening block.
...
- If the README has an obvious unmarked language switcher near the top, normalize it in place rather
  than inserting a second selector elsewhere.
...
If the repository already uses another multilingual naming scheme, preserve that existing scheme
