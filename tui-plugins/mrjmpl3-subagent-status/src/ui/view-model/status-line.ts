...
const renderAggregate = (counts: SubagentCounts): string =>
  `Subagents: ${counts.running} run · ${counts.done} done · ${counts.error} err`;
...
export const renderStatusLine = (state: SubagentState, nowMs = Date.now()): string => {
...
export const renderStatusSnapshotLine = (state: SubagentState, nowMs = Date.now()): string => {
  const view = buildSubagentSnapshotView(Object.values(state.children), nowMs);
