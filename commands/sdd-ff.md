...
orchestrator preflight prompt and STOP. Do not launch planning sub-agents in the same turn.
...
- In `interactive` mode: run only the next planning phase, present its summary and artifact path(s),
  ask whether to adjust or continue, then STOP. Do not launch the following phase until the user
  confirms.
- In `auto` mode: run all planning phases back-to-back and present a combined summary after all
  phases complete.
...
engram/hybrid, each phase saves with topic_key "sdd/$ARGUMENTS/{type}" where type is: proposal,
spec, design, tasks.
