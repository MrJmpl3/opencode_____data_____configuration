...
- [x] 2.2 Update `src/reconcile.ts` so completion/error evidence terminalizes rows, delegation rows
      stay out of totals, and duplicate fallback/session executions count once.
- [x] 2.3 Wire recovery ordering in `src/refresh.ts`: SQLite/JSON recovery first, then token
      hydration, then prune.

## Phase 3: Testing / Verification

- [x] 3.1 Extend `src/state.test.ts` with done→running regression, frozen elapsed-time, and
      prune/non-resurrection cases.
- [x] 3.2 Extend `src/reconcile.test.ts` with terminal evidence promotion, delegation exclusion, and
      duplicate execution dedupe cases.
- [x] 3.3 Add `src/refresh.test.ts` coverage for SQLite-backed hydration and stale-local override,
      plus a new recovery-focused test file if needed.
