...
function logWarn(client: OpencodeClient | undefined, service: string, message: string): void {
  if (!client) {
...
async function getProjectId(projectRoot: string): Promise<string> {
  ...
  if (gitStat.isFile()) {
    ...
    if (!match) throw new Error(`getProjectId: .git file exists but has invalid format at ${gitPath}`);
    ...
    if (!gitDirStat?.isDirectory()) throw new Error(`getProjectId: Resolved gitdir ${gitDir} is not a directory`);
  }
...
{"title": "Your Title Here", "description": "Your description here."}`;
    ...
    await debugLog(`generateMetadata error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return fallbackMetadata();
  }
}
 ...
 */
function formatDelegationContext(running: DelegationForContext[], completed: DelegationForContext[]): string {
