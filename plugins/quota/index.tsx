...
function View(props: {
  ...
  return (
    <box gap={0}>
      ...
      <Show when={props.getLines().length > 0} fallback={
        <text fg={theme().textMuted} wrapMode="none">No data</text>
      }>
        {props.getLines().map((line) => (
          <text fg={theme().textMuted} wrapMode="none">{line}</text>
        ))}
      </Show>
    </box>
  )
...
const plugin: TuiPluginModule & { id: string } = {
  ...
  tui: async (api) => {
    ...
    async function refresh(source?: string) {
      ...
      function buildLines() {
        const items: string[] = []
        for (const [tag, key] of [["OpenCode Go", "go"], ["GitHub Copilot", "cp"], ["OpenRouter", "or"]] as const) {
      ...
      try {
        ...
        if (goConfig) {
          ...
          if ("data" in result) {
            const d = result.data
            const lines: string[] = []
            for (const [name, key] of [["5h Rolling", "rolling"], ["Weekly", "weekly"], ["Monthly", "monthly"]] as const) {
              const w = d[key]
              if (w) lines.push(`${name}  ${w.remaining.toFixed(0)}%  · ${fmtDuration(w.resetInSec)} left`)
            }
        ...
        // ── Toast ──
        const toastMsg = firstError ? `Quota: ${firstError}` : `Quota updated${source ? ` (${source})` : ""}`
        try { (api as any).ui.toast(toastMsg) } catch {}

      } catch (e) {
        if (disposed || currentVersion !== inFlightVersion) return
        const msg = `Error: ${e instanceof Error ? e.message : String(e)}`
        setLines([msg])
        try { (api as any).ui.toast(`Quota: ${msg}`) } catch {}
      }
    }
    ...
    slots.register({
      ...
      slots: {
        sidebar_content() {
          return <View getLines={lines} getCountdown={countdownText} api={api} />
        },
    }});
  },
}
