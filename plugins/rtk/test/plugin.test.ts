  ...
  const shell = ((strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    if (command.startsWith("rtk rewrite ")) {
      ...
      const runner: ShellRunner = {
        nothrow: async () => ({ stdout: rewrite ? rewrite(originalCommand) : originalCommand }),
        quiet: () => runner,
      }
...
describe("RtkOpenCodePlugin", () => {
  ...
  it("rewrites bash commands when rtk returns a different command", async () => {
    const { executeBefore } = await createHook({ rewrite: () => "rtk git status --short" })
    const args = { command: "git status --short" }

    await executeBefore?.({ callID: "call", sessionID: "session", tool: "bash" }, { args } as never)

    expect(args.command).toBe("rtk git status --short")
  })

  it("ignores non-bash tools", async () => {
    const { calls, executeBefore } = await createHook({ rewrite: () => "changed" })
    const args = { command: "git status --short" }

    await executeBefore?.({ callID: "call", sessionID: "session", tool: "read" }, { args } as never)
  ...
  it("keeps the original command when rewrite fails", async () => {
    const { executeBefore } = await createHook({ rewrite: () => { throw new Error("fail") } })
    const args = { command: "git status --short" }
