...
const validateEdit = (file: ParsedFile, edit: NormalizedEdit) => {
  ...
  if (edit.op === "replace") {
    ...
    if (edit.end.line !== edit.pos.line) {
      throw new Error(
        "Multiline replace is not supported. Use one-line replace or multiple single-line edits.",
      );
    }
...
export const validateEditBatch = (
  ...
  for (const edit of edits) {
    ...
    if (line !== containingRange.start) {
      throw new Error(
        `${edit.op} on ${edit.pos?.raw ?? edit.end?.raw} conflicts with replace ${containingRange.raw}. Use its first line.`,
      );
    }
  }
};
