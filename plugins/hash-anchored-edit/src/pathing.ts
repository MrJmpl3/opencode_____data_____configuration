...
export const writeTextFileAtomically = async (filePath: string, content: string) => {
  ...
  const tempFilePath = path.join(path.dirname(filePath), `.${path.basename(filePath)}.${randomUUID()}.tmp`)
...
export const renderDirectory = async (filePath: string) => {
  ...
  return entries
    ...
    .map((entry: { isDirectory: () => boolean; name: string }) => `${entry.name}${entry.isDirectory() ? "/" : ""}`)
    .join("\n")
}
